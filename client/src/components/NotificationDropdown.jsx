...
export const NotificationDropdown = ({ user }) => {
    ...
    // Check for urgent / today's tasks to add real-time dynamic notifications
    useEffect(() => {
            ...
            try {
                    ...
                    if (task.dueDate) {
                        ...
                        if (isOverdue) {
                            urgentNotifications.push({
                                ...
                                message: `Hạn chót là ngày ${new Date(task.dueDate).toLocaleDateString('vi-VN')}. Hãy hoàn thành sớm!`,
                            ...
                            urgentNotifications.push({
                                ...
                                message: 'Nhiệm vụ này cần được hoàn thành trong ngày hôm nay.',
                                time: new Date().toISOString(),
                                isRead: false,
                                priority: 'high',
                                taskId: task._id || task.id
                ...
                if (urgentNotifications.length > 0) {
                    setNotifications(prev => {
                        ...
                        const newOnes = urgentNotifications.filter(n => !existingIds.has(n.id));
                        if (newOnes.length > 0) {
                            return [...newOnes, ...prev];
                        }
                        return prev;
    ...
    const filteredNotifications = notifications.filter(n => {
        ...
        if (filter === 'reminders') return n.type === 'reminder' || n.type === 'alert';
        return true;
    });
                            ...
                            filteredNotifications.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 pr-4">
                                        ...
                                        <span className="text-[10px] text-gray-400 mt-1.5 inline-block font-medium">
                                    ...
                                    <button
                                        ...
                                        className="opacity-0 group-hover:opacity-100 p-1 text-gray-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all absolute top-3 right-2 cursor-pointer"
                                        title="Xóa thông báo"
                                    >
