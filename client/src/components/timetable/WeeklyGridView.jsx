...
export const WeeklyGridView = ({
    ...
    const getClassesForSlotAndDay = (slotItem, dayKey) => {
        return filteredItems.filter(item => {
            ...
            // Match strictly by startTimes so each class only ever appears in exactly 1 slot row
                        ...
                        <tr className="bg-pink-50/60 border-b border-pink-100">
                            ...
                            {DAYS_OF_WEEK.map(day => {
                                ...
                                return (
                                    <th 
                                        key={day.key} 
                                        className={`p-3.5 text-xs font-bold uppercase tracking-wider text-center border-l border-pink-100/60 transition-colors ${
                                            isToday 
                                ...
                                >
                                    ...
                                    {DAYS_OF_WEEK.map(day => {
                                        ...
                                        return (
                                            <td 
                                                key={day.key}
                                                className={`p-2 border-l border-pink-100/60 align-top transition-colors ${
                                                    isToday ? 'bg-pink-100/20' : ''
                                                }`}
                                            >
                                                {classes.length === 0 ? (
                                                    <div className="h-14 flex items-center justify-center text-gray-200 text-xs">
                                                        -
                                                    </div>
                                                ) : (
                                                    <div className="space-y-1.5">
                                                        {classes.map(cls => {
                                                            ...
                                                            const locStyle = LOCATION_CONFIG[cls.location];

                                                            return (
                                                                <button
                                                                    key={cls.id}
                                                                    onClick={() => onSelectClass && onSelectClass(cls)}
                                                                    className={`w-full text-left p-2.5 rounded-2xl bg-gradient-to-br ${gradeStyle.cardBg} border border-pink-200/80 shadow-xs hover:shadow-md hover:scale-[1.02] transition-all cursor-pointer group relative overflow-hidden`}
                                                                >
                                                                    {/* Left accent bar */}
